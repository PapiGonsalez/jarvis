...
**Phase: P5 (wire calendar tile) shipped → P6 (wire ideas tile) next**
...
- **P6 (next):** Wire the Ideas tile against `ideas/` (manual entry, list + status). Reuses the FastAPI helper. Lighter than P5 because the data source is a local folder (no OAuth, no recurring expansion). Decisions to make: read-only vs allow add/edit from the tile, status taxonomy (active / paused / done? or different), sort order, and whether the tile should also surface idea-related notes.
